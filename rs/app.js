...
CB = {
    ...
    write: {
        ...
        input: function () {
            ...
            function autosize(e) {
                ...
                setTimeout(function(){
                    CB.write.keypad();
                    _me.css({height: 'auto'}).css({height: _me.prop('scrollHeight')});
                }, 0);
            }
            ...
            function submithandle() {
                var _space = $(_el.textareaClass).val().replace(/\s|　/gi, '');
    ...
    message: {
                        ...
                        setTimeout(function () {
                            ...
                            if (i == responseData.length -1) {
                                ...
                                if (responseData[i].is_next_feedback == 1) {
                                    ...
                                    html += '<li><button type="button" class="btn-feedback icon like" data-key="300" feedback-type="like"></button></li>';
                                    html += '<li><button type="button" class="btn-feedback icon unlike" data-key="301" feedback-type="unlike"></button></li>';
                                    html += '</ul>';
                                }
                                html += '</div>';
                                html += '</div>';
        ...
        render: function (_data, _key, _type, _delay, _hotkey) {
            ...
            setTimeout(function () {
                ...
                for (var i = 0; i < _data.length; i++) {
                    // 렌더링시에 PC 구분하여 PC 일 경우 카메라 버튼 제거(다만 li 레벨에서 삭제가 진행되어야 빈칸이 없으므로 상단에서 분리)
                    if(CB.settings.deviceStr == "pc" && _key[i] == 'camera') {
                        // li 없이 다음 element로 패스
                    } else {
                        html += '<li>';
                        if (_key[i] == '300') {
	  ...
	  if(CB.settings.deviceStr != 'pc'){
                ...
                $(_el.layerClass).find('img').attr('src', imageUri);
                setTimeout(function(){
                    if(windowW < $(_el.layerClass).find('img').width()){
                        $(_el.layerClass).find('img').css({width: 'auto'});
                    }
                    if(windowH < $(_el.layerClass).find('img').height()){
                        $(_el.layerClass).find('img').css({height: 'auto'});
                    }
            ...
            $(_el.layerClass).on('click', _el.closeClass, function () {
                ...
                $(_el.layerClass).find('.image-cont').removeClass('horizontal vertical');
